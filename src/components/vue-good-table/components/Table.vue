      ...
      <table ref="table" :class="tableStyleClasses">
        ...
        <tr v-if="globalSearch && externalSearchQuery == null">
          <td :colspan="lineNumbers ? columns.length + 1: columns.length">
            <div class="global-search">
              ...
              <input type="text" class="form-control global-search-input" :placeholder="globalSearchPlaceholder"
                     v-model="globalSearchTerm" @keyup.enter="searchTable()"/>
            </div>
          </td>
        </tr>
        <tr>
        ...
        <tr v-if="hasFilterRow">
          ...
          <th v-for="(column, index) in columns"
              :key="index"
              v-if="!column.hidden">
            <div v-if="column.filterable && column.filterField"
                 :class="getHeaderClasses(column, index)">
              <input v-if="!column.filterDropdown"
                     type="text"
                     class=""
                     :placeholder="getPlaceholder(column)"
                     v-model="filterParams[column.filterField]"
        ...
        <tr
          ...
          <slot name="table-row" :row="row" :formattedRow="formattedRow(row)" :collect="collect" :index="index">
            <td
              v-for="(column, i) in columns"
              :key="i"
              :class="getClasses(i, 'td', column)"
...
export default {
  ...
  methods: {
    ...
    updateFilters (column, value, isDropdown) {
      if (value == -1 && isDropdown) {
        this.$emit('on-filter', column, false)
        return
      }
      this.$emit('on-filter', column, value)
      //        const _this = this;
...
//        return computedRows
  ...
  computed: {
    ...
    processedRows () {
      ...
      //
      //            // built in sort
      //            let typeDef = this.typedColumns[this.sortColumn].typeDef;
  ...
  mounted () {
    ...
    //      if (this.startSort) {
    //        let columnIndex = this.columns.findIndex(column => {
    //          let field = column.filterField ? column.filterField : column.filter
    //          return field == this.startSort
    //        })
    //
    //        if(columnIndex >= 0){
    //
    //        }
    //
    //        if (this.startSort.orderby) {
    //
    //        }
    //        console.log(this.startSort)
    //      }
